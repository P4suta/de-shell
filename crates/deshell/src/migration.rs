...
fn rust_pattern_test(
    ...
    Ok(match pattern_shape(pattern) {
        ...
        PatternShape::Contains(infix) => format!("{subject}.contains({infix:?})"),
        PatternShape::Prefix(prefix) => format!("{subject}.starts_with({prefix:?})"),
        PatternShape::Suffix(suffix) => format!("{subject}.ends_with({suffix:?})"),
        // The length test is what stops `ab` from matching `a*b` by using its
        // one `b` as both ends.
        PatternShape::PrefixAndSuffix { prefix, suffix } => format!(
            "{subject}.len() >= {} && {subject}.starts_with({prefix:?}) && {subject}.ends_with({suffix:?})",
        ...
        PatternShape::FixedGap {
            ...
            "{subject}.chars().count() == {} && {subject}.starts_with({prefix:?}) && {subject}.ends_with({suffix:?})",
...
fn generate_go(plan: &crate::ir::Plan) -> Result<Vec<u8>, String> {
    ...
    let (pattern_strings, pattern_utf8) = go_pattern_packages(&task.body);
