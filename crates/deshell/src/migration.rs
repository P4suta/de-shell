...
fn generate_rust(plan: &crate::ir::Plan) -> Result<Vec<u8>, String> {
    ...
    let pattern_helper = if node_uses_pattern_matcher(&task.body) {
        concat!(
            "/// One piece of a shell `case` pattern.\n",
            "///\n",
            "/// A struct with a numbered kind rather than an enum: a plan that\n",
            "/// uses `*` and not `?` would leave an enum variant unconstructed,\n",
            "/// which this program's own `-D warnings` build rejects. The\n",
            "/// alternative was to suppress the warning, which would keep the\n",
            "/// unused code rather than not emit it.\n",
            "struct DeshellPiece<'a> {\n",
            "    /// 0 is a literal run, 1 is `*`, 2 is `?`.\n",
            "    kind: u8,\n",
            "    text: &'a str,\n",
            "}\n\n",
            "/// Match a shell `case` pattern.\n",
            "///\n",
            "/// `*` tries every split rather than the longest one: a\n",
            "/// left-to-right scan gets `*a*a` wrong. In a `case` a `*` crosses\n",
            "/// a `/`, unlike the same character in a filename glob, so there is\n",
            "/// nothing to exclude.\n",
            "fn deshell_pattern_matches(pieces: &[DeshellPiece<'_>], subject: &str) -> bool {\n",
            "    let Some((first, rest)) = pieces.split_first() else {\n",
            "        return subject.is_empty();\n",
            "    };\n",
            "    match first.kind {\n",
            "        0 => match subject.strip_prefix(first.text) {\n",
            "            Some(remainder) => deshell_pattern_matches(rest, remainder),\n",
            "            None => false,\n",
            "        },\n",
            "        2 => match subject.chars().next() {\n",
            "            Some(character) => {\n",
            "                deshell_pattern_matches(rest, &subject[character.len_utf8()..])\n",
            "            }\n",
            "            None => false,\n",
            "        },\n",
            "        _ => std::iter::once(0)\n",
            "            .chain(subject.char_indices().map(|(i, c)| i + c.len_utf8()))\n",
            "            .any(|split| deshell_pattern_matches(rest, &subject[split..])),\n",
            "    }\n",
            "}\n\n"
        )
    } else {
    ...
    let variable_binding = if !uses_variables {
        String::new()
    } else if rust_node_sets_variables(&task.body) {
        "    let mut deshell_vars: std::collections::BTreeMap<String, String> =\n        std::collections::BTreeMap::new();\n"
            .to_owned()
    } else {
        // Read-only: `mut` would be an unused-mut warning, and the generated Rust
        // is checked with `-D warnings`.
...
///
/// A pattern with no metacharacter becomes an equality test, which is what it
/// is; one with a `*` or a `?` becomes a call to the matcher the preamble
/// defines. The two are not the same check, and which one applies was decided
/// by where the quoting was — `'a*c'` is equality, `"a"*"c"` is a match.
fn rust_pattern_test(
    pattern: &crate::ir::PatternExpression,
    subject: &str,
) -> Result<String, String> {
    if let Some(exact) = pattern.exact() {
        return Ok(format!("{subject} == {exact:?}"));
    }
    let mut pieces = Vec::new();
    for piece in &pattern.pieces {
        pieces.push(match piece {
            crate::ir::PatternPiece::Literal { value } => format!(
                "DeshellPiece {{ kind: 0, text: &{} }}",
                rust_expression(value)?
            ),
            crate::ir::PatternPiece::AnyRun => "DeshellPiece { kind: 1, text: \"\" }".to_owned(),
            crate::ir::PatternPiece::AnyCharacter => {
                "DeshellPiece { kind: 2, text: \"\" }".to_owned()
            }
        });
    }
    Ok(format!(
        "deshell_pattern_matches(&[{}], &{subject})",
        pieces.join(", ")
    ))
}
...
fn go_pattern_test(
    ...
    if let Some(exact) = pattern.exact() {
        return Ok(format!("{subject} == {}", go_string(&exact)?));
    }
    let mut pieces = Vec::new();
    for piece in &pattern.pieces {
        pieces.push(match piece {
            crate::ir::PatternPiece::Literal { value } => {
                format!("{{kind: 0, text: {}}}", go_expression(value)?)
            }
            crate::ir::PatternPiece::AnyRun => "{kind: 1}".to_owned(),
            crate::ir::PatternPiece::AnyCharacter => "{kind: 2}".to_owned(),
        });
    }
    Ok(format!(
        "deshellPatternMatches([]deshellPiece{{{}}}, {subject})",
        pieces.join(", ")
    ))
}

/// Whether any `case` under this node needs the pattern matcher.
///
/// Written over the shared walk for the same reason the pipeline check is: a
/// hand-listed few operations missed a `case` inside a loop, and the generated
/// program called a function the generator had decided not to define.
fn node_uses_pattern_matcher(node: &crate::ir::Node) -> bool {
    if let crate::ir::Operation::Match { cases, .. } = &node.operation
        && cases.iter().any(|case| case.pattern.exact().is_none())
    {
        return true;
    }
    let mut found = false;
    visit_node(node, |child| found |= node_uses_pattern_matcher(child));
    found
}
...
fn generate_go(plan: &crate::ir::Plan) -> Result<Vec<u8>, String> {
    ...
    let pattern_helper = if node_uses_pattern_matcher(&task.body) {
        concat!(
            "type deshellPiece struct {\n",
            "\tkind int\n",
            "\ttext string\n",
            "}\n\n",
            "// deshellPatternMatches matches a shell `case` pattern. Kind 0 is a\n",
            "// literal run, 1 is `*` and 2 is `?`. `*` tries every split rather\n",
            "// than the longest one, because a left-to-right scan gets `*a*a`\n",
            "// wrong; in a `case` it also crosses a `/`, unlike a filename glob.\n",
            "func deshellPatternMatches(pieces []deshellPiece, subject string) bool {\n",
            "\tif len(pieces) == 0 {\n\t\treturn subject == \"\"\n\t}\n",
            "\tfirst, rest := pieces[0], pieces[1:]\n",
            "\tswitch first.kind {\n",
            "\tcase 0:\n",
            "\t\tif !strings.HasPrefix(subject, first.text) {\n\t\t\treturn false\n\t\t}\n",
            "\t\treturn deshellPatternMatches(rest, subject[len(first.text):])\n",
            "\tcase 2:\n",
            "\t\tif subject == \"\" {\n\t\t\treturn false\n\t\t}\n",
            "\t\t_, width := utf8.DecodeRuneInString(subject)\n",
            "\t\treturn deshellPatternMatches(rest, subject[width:])\n",
            "\tdefault:\n",
            "\t\tfor split := 0; split <= len(subject); split++ {\n",
            "\t\t\tif split > 0 && !utf8.RuneStart(subject[split-1]) {\n\t\t\t\tcontinue\n\t\t\t}\n",
            "\t\t\tif deshellPatternMatches(rest, subject[split:]) {\n\t\t\t\treturn true\n\t\t\t}\n",
            "\t\t}\n",
            "\t\treturn false\n",
            "\t}\n",
            "}\n\n"
    ...
    let go_matches = node_uses_pattern_matcher(&task.body);
    let imports = match (go_captures || go_matches, go_matches) {
        (true, true) => concat!(
            "import (\n\t\"fmt\"\n\t\"os\"\n\t\"os/exec\"\n\t\"strings\"\n",
            "\t\"unicode/utf8\"\n)\n\n"
        ),
